...
<?php $__env->startSection('title', 'Informe Técnico'); ?>
<?php $__env->startSection('page-title', 'Informe Técnico del Servicio'); ?>
        ...
        <div class="bg-gradient-to-r from-blue-600 to-blue-800 text-white p-4 sm:p-6 md:p-8 rounded-lg mb-4 sm:mb-6 md:mb-8 shadow-lg">
            <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3 sm:gap-4">
                <div>
                    ...
                    <p class="text-sm sm:text-base text-blue-100">Servicio #<?php echo e($servicio->id); ?></p>
                </div>
                <div class="text-left sm:text-right">
                    <p class="text-lg sm:text-xl md:text-2xl font-bold"><?php echo e(now()->format('d/m/Y H:i')); ?></p>
                    <p class="text-xs sm:text-sm text-blue-100">Generado: <?php echo e(now()->format('H:i')); ?></p>
                </div>
            </div>
        </div>

        <form id="informe-form" action="<?php echo e(route('incidencias.servicios.store-report', $servicio)); ?>" method="POST" enctype="multipart/form-data" class="space-y-4 sm:space-y-6 md:space-y-8">
            <?php echo csrf_field(); ?>
            ...
            <div class="bg-white shadow-lg rounded-lg p-4 sm:p-6 md:p-8">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 md:gap-6">
                    ...
                    <div class="bg-blue-50 p-3 sm:p-4 rounded-lg">
                        ...
                        <p class="text-base sm:text-lg font-bold text-gray-900 mt-1 line-clamp-2"><?php echo e($servicio->equipo->area->sede->cliente->razon_social); ?></p>
                    </div>

                    <!-- NIT/Identificación -->
                    <div class="bg-blue-50 p-3 sm:p-4 rounded-lg">
                        <p class="text-xs sm:text-sm text-gray-600 font-semibold uppercase tracking-wider">NIT / Identificación</p>
                        <p class="text-base sm:text-lg font-bold text-gray-900 mt-1"><?php echo e($servicio->equipo->area->sede->cliente->documento_formateado ?? $servicio->equipo->area->sede->cliente->documento ?? 'N/A'); ?></p>
                    </div>

                    <!-- Sede -->
                    <div class="bg-blue-50 p-3 sm:p-4 rounded-lg">
                        <p class="text-xs sm:text-sm text-gray-600 font-semibold uppercase tracking-wider">Sede</p>
                        <p class="text-base sm:text-lg font-bold text-gray-900 mt-1"><?php echo e($servicio->equipo->area->sede->nombre); ?></p>
                    </div>

                    <!-- Dirección -->
                    <div class="bg-blue-50 p-3 sm:p-4 rounded-lg">
                        <p class="text-xs sm:text-sm text-gray-600 font-semibold uppercase tracking-wider">Dirección</p>
                        <p class="text-base sm:text-lg font-bold text-gray-900 mt-1 line-clamp-2"><?php echo e($servicio->equipo->area->sede->direccion ?? 'N/A'); ?></p>
                    </div>

                    <!-- Teléfono -->
                    <div class="bg-blue-50 p-3 sm:p-4 rounded-lg">
                        <p class="text-xs sm:text-sm text-gray-600 font-semibold uppercase tracking-wider">Teléfono</p>
                        <p class="text-base sm:text-lg font-bold text-gray-900 mt-1"><?php echo e($servicio->equipo->area->sede->telefono ?? 'N/A'); ?></p>
                    </div>

                    <!-- Ciudad -->
                    <div class="bg-blue-50 p-3 sm:p-4 rounded-lg">
                        <p class="text-xs sm:text-sm text-gray-600 font-semibold uppercase tracking-wider">Ciudad</p>
                        <p class="text-base sm:text-lg font-bold text-gray-900 mt-1"><?php echo e($servicio->equipo->area->sede->municipio->nombre ?? $servicio->equipo->area->sede->cliente->ciudadNotificacion->nombre ?? 'N/A'); ?></p>
                    </div>
                    ...
                    <div class="bg-green-50 p-3 sm:p-4 rounded-lg col-span-1 sm:col-span-2 lg:col-span-3">
                        ...
                        <p class="text-base sm:text-lg font-bold text-gray-900 mt-1"><?php echo e($servicio->contrato->numero_contrato ?? 'Sin Contrato'); ?> <?php if($servicio->contrato): ?> <span class="text-sm">(<?php echo e($servicio->contrato->descripcion); ?>)</span> <?php endif; ?></p>
                    </div>
                </div>
            </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 md:gap-6">
                    ...
                    <div>
                        ...
                        <div class="bg-gray-100 p-3 rounded-lg border border-gray-300">
                            <p class="font-semibold text-gray-900"><?php echo e($servicio->fecha_solicitud->format('d/m/Y H:i')); ?></p>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="date" name="fecha_atencion" 
                            ...
                            value="<?php echo e(old('fecha_atencion', $servicio->fecha_atencion ? $servicio->fecha_atencion->format('Y-m-d') : '')); ?>"
                            required>
                        <?php $__errorArgs = ['fecha_atencion'];
...
unset($__errorArgs, $__bag); ?>
                    ...
                    <div>
                        ...
                        <input type="time" name="hora_inicio_atencion" 
                            class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-purple-500"
                            value="<?php echo e(old('hora_inicio_atencion', $servicio->hora_inicio_atencion ? \Carbon\Carbon::parse($servicio->hora_inicio_atencion)->format('H:i') : '')); ?>"
                            required>
                        <?php $__errorArgs = ['hora_inicio_atencion'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?>
...
unset($__errorArgs, $__bag); ?>
                    ...
                    <div>
                        ...
                        <input type="time" name="hora_fin_atencion" 
                            class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-purple-500"
                            value="<?php echo e(old('hora_fin_atencion', $servicio->hora_fin_atencion ? \Carbon\Carbon::parse($servicio->hora_fin_atencion)->format('H:i') : '')); ?>"
                            required>
                        <?php $__errorArgs = ['hora_fin_atencion'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4 md:gap-6">
                    ...
                    <div>
                        ...
                        <select name="tipo_servicio_informe" id="tipo_servicio_informe"
                            ...
                            <option value="INSTALACION" <?php if(old('tipo_servicio_informe', $servicio->tipo_servicio_informe) === 'INSTALACION'): ?> selected <?php endif; ?>>🔌 Instalación</option>
                            <option value="MANTENIMIENTO_PREVENTIVO" <?php if(old('tipo_servicio_informe', $servicio->tipo_servicio_informe) === 'MANTENIMIENTO_PREVENTIVO'): ?> selected <?php endif; ?>>🛡️ Mantenimiento Preventivo</option>
                            <option value="MANTENIMIENTO_CORRECTIVO" <?php if(old('tipo_servicio_informe', $servicio->tipo_servicio_informe) === 'MANTENIMIENTO_CORRECTIVO'): ?> selected <?php endif; ?>>⚙️ Mantenimiento Correctivo</option>
                            <option value="SOPORTE" <?php if(old('tipo_servicio_informe', $servicio->tipo_servicio_informe) === 'SOPORTE'): ?> selected <?php endif; ?>>🆘 Soporte</option>
                        </select>
                        <?php $__errorArgs = ['tipo_servicio_informe'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?>
                    ...
                    <div class="flex items-end">
                        <label class="flex items-center gap-3 p-3 border-2 border-green-300 rounded-lg cursor-pointer hover:bg-green-50 flex-1">
                            <input type="checkbox" name="puede_facturarse" value="1"
                                ...
                                <?php if(old('puede_facturarse', $servicio->puede_facturarse)): ?> checked <?php endif; ?>>
                            <div>
            ...
            <div class="bg-white shadow-lg rounded-lg p-4 sm:p-6 md:p-8">
                ...
                <div class="overflow-x-auto border rounded-lg">
                    <table class="min-w-full text-sm">
                        ...
                        <tbody>
                            <?php $__empty_1 = true; $__currentLoopData = $equiposAdicionales; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $equipoUbicacion): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                <?php
                                    $esPrincipal = $equipoUbicacion->id === $servicio->equipo_id;
                                ?>
                                <tr class="border-t <?php echo e($esPrincipal ? 'bg-blue-50' : 'bg-white'); ?>">
                                    <td class="px-3 py-2">
                                        <?php if($esPrincipal): ?>
                                            <span class="inline-block px-2 py-1 text-xs rounded bg-blue-100 text-blue-800 font-semibold">Principal</span>
                                        <?php else: ?>
                                            <input type="checkbox" name="equipos_adicionales[]" value="<?php echo e($equipoUbicacion->id); ?>"
                                                class="w-4 h-4"
                                                <?php echo e(in_array($equipoUbicacion->id, $equiposSeleccionados, true) ? 'checked' : ''); ?>>
                                        <?php endif; ?>
                                    </td>
                                    <td class="px-3 py-2 font-semibold text-gray-900"><?php echo e($equipoUbicacion->codigo_activo_cliente ?? ('EQ-' . $equipoUbicacion->id)); ?></td>
                                    <td class="px-3 py-2 text-gray-700"><?php echo e($equipoUbicacion->tipoEquipo->nombre ?? 'N/A'); ?></td>
                                    <td class="px-3 py-2 text-gray-700"><?php echo e($equipoUbicacion->marca->nombre ?? 'N/A'); ?> <?php echo e($equipoUbicacion->modelo ?? ''); ?></td>
                                    <td class="px-3 py-2 text-gray-700"><?php echo e($equipoUbicacion->serial ?? 'N/A'); ?></td>
                                    <td class="px-3 py-2 text-gray-700"><?php echo e($equipoUbicacion->contrato->numero_contrato ?? 'Sin contrato'); ?></td>
                                    <td class="px-3 py-2 text-gray-700"><?php echo e($equipoUbicacion->estado_operativo ?? 'N/A'); ?></td>
                                </tr>
                ...
                <?php $__errorArgs = ['equipos_adicionales'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?>
...
unset($__errorArgs, $__bag); ?>
            ...
            <div class="bg-white shadow-lg rounded-lg p-4 sm:p-6 md:p-8">
                ...
                <div>
                    ...
                    <div class="w-full border border-gray-300 rounded-lg px-4 py-3 bg-gray-100 text-gray-800 min-h-[96px] whitespace-pre-wrap"><?php echo e($servicio->descripcion_problema ?? 'N/A'); ?></div>
                </div>
            </div>
            ...
            <div class="bg-white shadow-lg rounded-lg p-4 sm:p-6 md:p-8">
                ...
                <div>
                    ...
                    <textarea name="diagnostico_validacion" id="diagnostico_validacion" rows="4"
                        ...
                        required><?php echo e(old('diagnostico_validacion', $servicio->diagnostico_validacion)); ?></textarea>
                    ...
                    <?php $__errorArgs = ['diagnostico_validacion'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?>
            ...
            <div class="bg-white shadow-lg rounded-lg p-4 sm:p-6 md:p-8">
                ...
                <div>
                    ...
                    <textarea name="observaciones_informe" id="observaciones_informe" rows="3"
                        ...
                        placeholder="Notas, recomendaciones, comentarios..."><?php echo e(old('observaciones_informe', $servicio->observaciones_informe)); ?></textarea>
                    <p class="text-xs text-gray-500 mt-1" data-dictation-status-for="observaciones_informe"></p>
                    <?php $__errorArgs = ['observaciones_informe'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
                    <div>
                        ...
                        <input type="text" name="persona_receptora_nombre" id="persona_receptora_nombre"
                            ...
                            value="<?php echo e(old('persona_receptora_nombre', $servicio->persona_receptora_nombre)); ?>"
                            required>
                        <?php $__errorArgs = ['persona_receptora_nombre'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?>
...
unset($__errorArgs, $__bag); ?>
                    ...
                    <div>
                        ...
                        <input type="text" name="persona_receptora_apellido" id="persona_receptora_apellido"
                            class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-pink-500"
                            value="<?php echo e(old('persona_receptora_apellido', $servicio->persona_receptora_apellido)); ?>"
                            required>
                        <?php $__errorArgs = ['persona_receptora_apellido'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?>
...
unset($__errorArgs, $__bag); ?>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="persona_receptora_documento" id="persona_receptora_documento"
                            class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-pink-500"
                            value="<?php echo e(old('persona_receptora_documento', $servicio->persona_receptora_documento)); ?>"
                            required>
                        <?php $__errorArgs = ['persona_receptora_documento'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?>
...
unset($__errorArgs, $__bag); ?>
            ...
            <div class="bg-white shadow-lg rounded-lg p-4 sm:p-6 md:p-8">
                ...
                <select name="estado_servicio_id" id="estado_servicio_id" 
                    ...
                    <?php $__currentLoopData = $estadosDisponibles; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $estado): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <option value="<?php echo e($estado->id); ?>" 
                            <?php if($servicio->estado_servicio_id === $estado->id): ?> selected <?php endif; ?>>
                            <?php echo e($estado->nombre); ?>

                        </option>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </select>
                <?php $__errorArgs = ['estado_servicio_id'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?>
...
unset($__errorArgs, $__bag); ?>
            ...
            <div class="bg-white shadow-lg rounded-lg p-4 sm:p-6 md:p-8">
                ...
                <?php $__errorArgs = ['firma_persona_receptora'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\CEOGestion\resources\views/incidencias/servicios/report-technician-v2.blade.php ENDPATH**/ ?>