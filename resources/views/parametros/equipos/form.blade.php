    ...
    <form action="{{ $equipo ? url('parametros/equipos/' . $equipo->id) : route('parametros.equipos.store') }}" method="POST" class="bg-white rounded-lg shadow p-6 space-y-6">
        ...
        <div class="border-b pb-6">
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <select id="empresa_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500">
                        ...
                        @foreach($empresas as $empresa)
                            <option value="{{ $empresa->id }}" {{ old('empresa_id', $equipo?->area?->sede?->empresa_id ?? '') == $empresa->id ? 'selected' : '' }}>
                                {{ $empresa->nombre }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="cliente_id" id="cliente_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500 @error('cliente_id') border-red-500 @enderror">
                        <option value="">-- Selecciona un cliente --</option>
                        @foreach($clientes as $cliente)
                            <option value="{{ $cliente->id }}" {{ old('cliente_id', $equipo?->cliente_id ?? $equipo?->area?->sede?->cliente_id ?? '') == $cliente->id ? 'selected' : '' }}>
            ...
            <div class="grid grid-cols-3 gap-4">
                <div>
                    ...
                    <select id="sede_id" name="sede_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500 @error('sede_id') border-red-500 @enderror">
                        <option value="">Seleccione sede</option>
                        @foreach($sedes as $sede)
                            <option value="{{ $sede->id }}" 
                                data-empresa-id="{{ $sede->empresa_id }}"
                                data-cliente-id="{{ $sede->cliente_id }}"
                                {{ old('sede_id', $equipo?->sede_id ?? $equipo?->area?->sede_id ?? '') == $sede->id ? 'selected' : '' }}>
                                {{ $sede->nombre }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="grid grid-cols-1 gap-4 mt-4">
                <div>
                    ...
                    <select id="contrato_id" name="contrato_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500 @error('contrato_id') border-red-500 @enderror">
                        <option value="">-- Sin contrato --</option>
                        @foreach($contratos as $contrato)
                            <option value="{{ $contrato->id }}" 
                                data-cliente-id="{{ $contrato->cliente_id }}"
                                data-numero="{{ $contrato->numero_contrato }}"
                                {{ old('contrato_id', $equipo?->contrato_id ?? '') == $contrato->id ? 'selected' : '' }}>
                                {{ $contrato->numero_contrato }} - {{ $contrato->cliente->razon_social }} ({{ $contrato->tipo_contrato }})
                            </option>
                        @endforeach
                    </select>
        ...
        <div class="border-b pb-6">
            ...
            <div class="grid grid-cols-3 gap-4">
                ...
                <div>
                    @error('serial')<span class="text-red-500 text-xs">{{ $message }}</span>@enderror
        ...
        <div class="border-b pb-6">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="marca_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500 @error('marca_id') border-red-500 @enderror" required>
                        <option value="">-- Seleccione marca --</option>
                        @foreach($marcas as $marca)
                            <option value="{{ $marca->id }}" {{ old('marca_id', $equipo->marca_id ?? '') == $marca->id ? 'selected' : '' }}>
                                {{ $marca->nombre }}
                            </option>
                        @endforeach
                    </select>
                    @error('marca_id')<span class="text-red-500 text-xs">{{ $message }}</span>@enderror
                </div>
