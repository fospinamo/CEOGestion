            ...
            <div class="space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <div class="flex gap-2">
                            <input type="color" name="color" value="{{ old('color', $categoria->color) }}" class="w-12 h-10 border border-gray-300 rounded-lg cursor-pointer" id="colorPicker">
                            <input type="text" id="colorInput" value="{{ old('color', $categoria->color) }}" class="flex-1 px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500 @error('color') border-red-500 @enderror" placeholder="#3b82f6">
                        </div>
                    ...
                    <div>
                        ...
                        <div class="flex gap-2">
                            <input type="text" name="icono" value="{{ old('icono', $categoria->icono) }}" class="flex-1 px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500 @error('icono') border-red-500 @enderror" placeholder="fa-desktop" id="iconoInput">
                            <div class="flex items-center px-4 py-2 bg-gray-100 rounded-lg">
                                <i id="iconoPreview" class="fas {{ $categoria->icono ?? 'fa-icon' }} text-xl text-gray-600"></i>
                            </div>
                        </div>
                ...
                <div>
                    <label class="flex items-center gap-3 cursor-pointer">
                        <input type="checkbox" name="estado" value="1" {{ old('estado', $categoria->estado) ? 'checked' : '' }} class="w-4 h-4 text-blue-600 rounded focus:ring-2 focus:ring-blue-500">
...
iconoInput.addEventListener('change', function() {
    iconoPreview.className = 'fas ' + (this.value || 'fa-icon') + ' text-xl text-gray-600';
});
</script>
@endsection
