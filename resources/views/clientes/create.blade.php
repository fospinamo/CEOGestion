...
@section('content')
<div class="max-w-4xl">
    <form action="{{ $cliente ? route('clientes.update', $cliente) : route('clientes.store') }}" method="POST" class="bg-white rounded-lg shadow p-6 space-y-6">
        @csrf
        @if($cliente)
            @method('PUT')
        @endif
        ...
        <div class="flex justify-end gap-4 pt-4">
            <a href="{{ route('clientes.index') }}" class="px-6 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 transition">
                Cancelar
            </a>
