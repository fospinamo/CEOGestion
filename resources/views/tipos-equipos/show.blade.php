...
<div class="grid grid-cols-3 gap-6">
    ...
    <div class="space-y-6">
        ...
        <div class="bg-white rounded-lg shadow p-6 space-y-2">
            <a href="{{ route('tipos-equipos.edit', $tipoEquipo) }}" class="w-full block text-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition font-semibold">
                <i class="fas fa-edit"></i> Editar
            </a>
            @if($tipoEquipo->equipos()->count() === 0)
                <form action="{{ route('tipos-equipos.destroy', $tipoEquipo) }}" method="POST" onsubmit="return confirm('¿Eliminar este tipo de equipo?')">
                    @csrf @method('DELETE')
